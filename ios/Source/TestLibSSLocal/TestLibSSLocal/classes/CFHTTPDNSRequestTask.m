...
static NSDictionary *sockesProxySetting = nil;
...
@property (nonatomic, copy) NSURL *realURL;          // 原始网络请求URL
@property (nonatomic, copy) NSURLRequest *swizzleRequest;           // DNS处理过后请求
...
- (CFHTTPDNSRequestTask *)initWithRealURL:(NSURL *)realURL swizzleRequest:(NSURLRequest *)swizzleRequest delegate:(id<CFHTTPDNSRequestTaskDelegate>)delegate {
    ...
    if (!realURL || !delegate || !swizzleRequest) {
        return nil;
    }
    ...
    if (self = [self init]) {
        ...
        NSLog(@"request: %@,%@", swizzleRequest.URL,swizzleRequest.allHTTPHeaderFields);
...
- (void)startLoading {
    ...
    if ([self isHTTPSScheme]) {
        ...
        NSLog(@"request: url:%@,headers:%@,%@,prop:%@",url,headFields,bodyData,sslProperties);
    }

    if (sockesProxySetting && headFields[@"X-SSLocal-Proxy"] ) { 
        
//        NSDictionary *proxySettings = @{
//            (NSString *)kCFStreamPropertySOCKSProxyHost: @"127.0.0.1",
//            (NSString *)kCFStreamPropertySOCKSProxyPort: @(10086), // 代理端口
//            (NSString *)kCFStreamPropertySOCKSVersion: (NSString *)kCFStreamSocketSOCKSVersion5 // 使用 SOCKS5
//        };

        NSLog(@"request,proxy:%@",sockesProxySetting);
        
        CFReadStreamSetProperty(readStream, kCFStreamPropertySOCKSProxy, (__bridge CFTypeRef)( sockesProxySetting.copy ));
    }
    ...
    if (result) {
        ...
        if (!added || ![added boolValue]) {
            ...
            if ([self needRedirection]) {
                ...
                NSHTTPURLResponse *response = [[NSHTTPURLResponse alloc] initWithURL:self.realURL statusCode:self.response.statusCode HTTPVersion:self.response.httpVersion headerFields:self.response.headerFields];
 ...
 */
- (BOOL)isHTTPSScheme {
    return [self.realURL.scheme isEqualToString:@"https"];
}
