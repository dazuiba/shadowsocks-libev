...
@interface SSNetURLProtocol () <NSStreamDelegate>
...
static id<SSNetURLProtocolHelper> g_protocolHelper;
@implementation SSNetURLProtocol
+ (void)setProtocolHelper:(id<SSNetURLProtocolHelper>)helper{
    g_protocolHelper = helper;
}
/**
 *  是否拦截处理指定的请求
 *
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request {
    ...
    if ([url hasPrefix:@"https"] && [g_protocolHelper shouldProcessDomain:domain]) {
        return YES;
    }
    return NO;
}
...
- (NSURL*)getIpAndReplace:(NSString*)urlString {
    ...
    NSArray* result = [g_protocolHelper getHostByName:url.host];
    NSString* ip = nil;
...
- (void)startRequest {
    ...
    NSDictionary *proxySettings = g_protocolHelper.connectionProxyDictionaryOrNil;
    if (proxySettings != nil) {
        CFReadStreamSetProperty(readStream, kCFStreamPropertySOCKSProxy, (__bridge CFTypeRef)(proxySettings));
    }
